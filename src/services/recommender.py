...
class RecommenderService:
    ...
    async def get_explanation(self, movie_titles: List[str], query: str) -> str:
        ...
        system_prompt = "Ты — интеллектуальный кино-ассистент, знаток кино."
        user_prompt = f"""
        Пользователь искал: "{query}"
        Я порекомендовал следующие фильмы: {titles_str}.
        
        Напиши короткое, но увлекательное объяснение на русском языке, почему эти фильмы подходят под запрос.
        Не перечисляй фильмы списком, а свяжи их в один связный текст.
